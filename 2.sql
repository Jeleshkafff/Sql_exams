...
ON `players` FOR EACH ROW BEGIN 
    DECLARE Date_of_Births Date;
    Set  @Date_of_Births = new.`Date_of_Birth`;
    IF (year(now()) - year(new.`Date_of_Birth`) <8)THEN
	   insert into player(Date_of_Birth) values (`Date_of_Births`);	
end if; 
	END // 
...
CREATE TRIGGER `team_check` before INSERT 
ON `team` FOR EACH ROW BEGIN 
    DECLARE DateTeam Date;
    Set  @DateTeam = new.`date_of_creation`;
    IF (year(now()) - year(new.`date_of_creation`) < 0)THEN
	   insert into teams(date_of_creation) values (`date_of_creation`);	
end if; 
	END // 
...
DROP TRIGGER IF EXISTS `play_team`;
DELIMITER //


CREATE TRIGGER `play_team` before INSERT 
ON `team_players` FOR EACH ROW BEGIN 
    DECLARE DateTeam Date;
...
DELIMITER ;
-- ---------------------------------------------------------------------
DROP TRIGGER IF EXISTS `doudle_insert`;
DELIMITER //
create trigger `doudle_insert` before INSERT 
ON `team_players` FOR EACH ROW BEGIN
...
INSERT INTO `Team_players` (`Player`,`Team`) VALUES();
