...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `play_team` BEFORE INSERT ON `team_players` FOR EACH ROW BEGIN 
    DECLARE DateTeam Date;
    Set  @DateTeam = new.`Date_of_membership`;
    IF (year(now()) - year(new.`Date_of_membership`) < 0)THEN
	   insert into team_player(Date_of_membership) values (`date_of_creation`);	
end if; 
	END */;;
DELIMITER ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `doudle_insert` BEFORE INSERT ON `team_players` FOR EACH ROW BEGIN
	DECLARE `Player_sel` INT UNSIGNED;
    DECLARE `Team_sel` INT UNSIGNED;
    Set  @Player_sel = new.Player;
    Set  @Team_sel = new.Team;
    if ( exists(select Player, Team from team_players 
    where Does_it_play= 1 and Player = new.Player 
    and Team = new.Team))THEN
    insert into team_player(Player,Team) values(Player_sel,Team_sel);
...
-- Dump completed on 2022-12-23  0:45:36
